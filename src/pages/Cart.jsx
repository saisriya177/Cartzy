...
const Cart = ({location, getLocation}) => {
  ...
  const totalPrice = Math.round(cartItem.reduce((total, item) => total + item.price, 0) , 2)
  return (
              ...
              {cartItem.map((item, index) => {
                return <div key={index} className='bg-gray-100 p-5 rounded-md flex items-center justify-between mt-3 w-full'>
                  <div className='flex items-center gap-4'>
                    <img src={item.thumbnail || item.images} alt={item.title} className='w-20 h-20 rounded-md' />
                    <div>
